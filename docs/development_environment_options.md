...
5. 背景、上位要求、下位要求、仕様の追加と、ブラウザダウンロードによる決定的な YAML 保存を実装する。
6. 親子関係は単一親とし、仕様の関連下位要求は YAML の参照情報として実装する。
7. ノード座標を保存せず、自動整列・自動配置を実装する。
8. macOS 向けに `Cmd+Z` と `Cmd+Y` の Undo / Redo を実装する。
9. 環境固定が必要になった段階で Dockerfile を追加する。

## テスト導入方針

- 初期段階は ID 採番、YAML 読み書き、検証、旧版 YAML 変換を Core 層の単体テストで確認する。
- キャンバス操作が安定するまでは UI を手動確認する。
- 主要操作が固まった段階で Playwright による UI 自動テストを追加する。
- 100、500、1,000ノード程度の測定用データで、描画と YAML 入出力を計測する。
