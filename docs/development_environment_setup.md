# USDM MindMap Editor Mac(M2) 開発環境構築手順書

作成日: 2026-07-15

## 1. 目的

本書は、USDM MindMap Editor を macOS / Apple Silicon(M2) 環境で開発・実行するための環境構築手順を定義する。

USDM MindMap Editor は、USDM の要求仕様をマインドマップ形式で整理・編集し、YAML 形式で保存する ASP.NET Core + Blazor Web App として開発する。
...
USDM MindMap Editor の Web アプリ本体は `.NET 10` を推奨する。
...
dotnet new sln -n USDM_Maker
dotnet new classlib -n USDM_Maker.Core -o src/USDM_Maker.Core -f net10.0
dotnet new blazor -n USDM_Maker.Web -o src/USDM_Maker.Web -f net10.0 --interactivity Server --auth None
dotnet new mstest -n USDM_Maker.Core.Tests -o tests/USDM_Maker.Core.Tests -f net10.0

dotnet sln add src/USDM_Maker.Core/USDM_Maker.Core.csproj
dotnet sln add src/USDM_Maker.Web/USDM_Maker.Web.csproj
dotnet sln add tests/USDM_Maker.Core.Tests/USDM_Maker.Core.Tests.csproj

dotnet add src/USDM_Maker.Web/USDM_Maker.Web.csproj reference src/USDM_Maker.Core/USDM_Maker.Core.csproj
dotnet add tests/USDM_Maker.Core.Tests/USDM_Maker.Core.Tests.csproj reference src/USDM_Maker.Core/USDM_Maker.Core.csproj
```
...
```zsh
dotnet add src/USDM_Maker.Core/USDM_Maker.Core.csproj package YamlDotNet
```
...
dotnet build
dotnet test
dotnet run --project src/USDM_Maker.Web
```
...
```zsh
dotnet publish src/USDM_Maker.Web/USDM_Maker.Web.csproj \
  -c Release \
  -r osx-arm64 \
  --self-contained true
```
